...
def map_role_to_place_type(role):
    role_place_type_correspondence = {
        "pup": "Town - historical",
        "mfp": "Town - historical",
        "uvp": "Town - historical",
        "Place of Making": "Town - historical",
    }
...
def map_role_to_person_type(role):
    role_person_type_correspondence = {
        "aut": "Author",
        "edt": "Author",
        "rsp": "Author",
        "prt": "Printer",
...
def map_person_to_connection_type(person):
    person_person_connection_type_correspondence = {
        "Vater": "father",
        "Mutter": "mother",
        "Bruder": "brother",
        "Schwester": "sister",
        ...
        "Ehemann": "husband",
        "1. Ehemann": "first husband",
        "2. Ehemann": "second husband",
        "3. Ehemann": "third husband",
        "4. Ehemann": "fourth husband",
        "5. Ehemann": "fifth husband",
        "6. Ehemann": "sixth husband",
        "Ehefrau": "wife",
        "1. Ehefrau": "first wife",
        "2. Ehefrau": "second wife",
        "3. Ehefrau": "third wife",
        "4. Ehefrau": "fourth wife",
        "5. Ehefrau": "fifth wife",
        "6. Ehefrau": "sixth wife",
        "Adoptivvater": "adoptive father",
        "Adoptivmutter": "adoptive mother",
        "Adoptivsohn": "adopted son",
        "Adoptivtochter": "adopted daughter",
...
def parse_relationship(
    ...
    if connection_comment:
        if (
            ("," in connection_comment or "(" in connection_comment)
            and connected_entity_type != "place"
        ):  # this seems to make little sense for connected places
            connection_comment_divided = re.match(
                connection_comment_pattern, connection_comment
            ).groups()
    ...
    if main_entity_type == "person":
        if connected_entity_type == "person":
            connection_type, connection_comment = (
                person_relations.gnd_person_person_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        elif connected_entity_type == "organisation":
            connection_type, connection_comment = (
                person_relations.gnd_person_org_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        elif connected_entity_type == "location":
            connection_type, connection_comment = (
                person_relations.gnd_person_place_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        ...
        if connected_entity_type == "person":
            connection_type, connection_comment = (
                person_relations.gnd_org_person_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        elif connected_entity_type == "organisation":
            connection_type, connection_comment = person_relations.gnd_org_org_relation(
                connection_type_raw, person_new_sex, connection_type
            )
        elif connected_entity_type == "location":
            connection_type, connection_comment = (
                person_relations.gnd_org_place_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        ...
        if connected_entity_type == "person":
            connection_type, connection_comment = (
                person_relations.gnd_place_person_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        elif connected_entity_type == "organisation":
            connection_type, connection_comment = (
                person_relations.gnd_place_org_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        elif connected_entity_type == "location":
            connection_type, connection_comment = (
                person_relations.gnd_place_place_relation(
                    connection_type_raw, person_new_sex, connection_type
                )
            )
        ...
        match roman_number[x]:
            ...
            case "C":
                if l - x > 2:  # if there are at least four characters following
                    if roman_number[x + 1] == "M" or (
                        roman_number[x + 1] == "C" and roman_number[x + 2] == "M"
                    ):
                        result = result - 100
                    elif roman_number[x + 1] == "D" or (
                        roman_number[x + 1] == "C" and roman_number[x + 2] == "D"
                    ):
                        result = result - 100
                    else:
                        result = result + 100
            ...
            case "X":
                if l - x > 2:  # if there are at least four characters following
                    if roman_number[x + 1] == "C" or (
                        roman_number[x + 1] == "X" and roman_number[x + 2] == "C"
                    ):
                        result = result - 10
                    elif roman_number[x + 1] == "L" or (
                        roman_number[x + 1] == "X" and roman_number[x + 2] == "L"
                    ):
                        result = result - 10
                    else:
                        result = result + 10
            ...
            case "I":
                if l - x > 2:  # if there are at least four characters following
                    if roman_number[x + 1] == "X" or (
                        roman_number[x + 1] == "I" and roman_number[x + 2] == "X"
                    ):
                        result = result - 1
                    elif roman_number[x + 1] == "V" or (
                        roman_number[x + 1] == "I" and roman_number[x + 2] == "V"
                    ):
                        result = result - 1
                    else:
                        result = result + 1
