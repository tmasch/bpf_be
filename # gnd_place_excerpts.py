...
def gnd_place_excerpts():
    with open(r"C:\Users\berth\bpf_files\gnd_persons\authorities-gnd-geografikum_dnbmarc_20240213.mrc.xml", "r", encoding = "utf-8") as f:
    ...
    #print(conn_place_list)
            ...
            if len(conn_person_list) > 100:
                with open(r"C:\Users\berth\bpf_files\gnd_place_persons.txt", "a", encoding = "utf-8") as g:
                    for person in conn_person_list:
                        g.writelines("#".join(person) + "\n")
                conn_person_list = []
            if len(conn_org_list) > 100:
                with open(r"C:\Users\berth\bpf_files\gnd_place_orgs.txt", "a", encoding = "utf-8") as h:
                    for org in conn_org_list:
                        h.writelines("#".join(org) + "\n")
                conn_org_list = []
            if len(dates_list) > 100:
                with open(r"C:\Users\berth\bpf_files\gnd_place_datestrings.txt", "a", encoding = "utf-8") as i:
                    for datestring in dates_list:
                        i.writelines(datestring + "\n")
                dates_list = []
                print("100 datestrings")
            if len(conn_date_list) > 100:            
                with open(r"C:\Users\berth\bpf_files\gnd_place_dates.txt", "a", encoding = "utf-8") as j:
                    for date in conn_date_list:
                        j.writelines("#".join(date) + "\n")
                conn_date_list = []
            if len(conn_keyword_list) > 100:
                with open(r"C:\Users\berth\bpf_files\gnd_place_keywords.txt", "a", encoding = "utf-8") as k:
                    for keyword in conn_keyword_list:
                        k.writelines("#".join(keyword) + "\n")
                conn_keyword_list = []
            if len(conn_place_list) > 100:
                with open(r"C:\Users\berth\bpf_files\gnd_place_places.txt", "a", encoding = "utf-8") as l:
