...
#                        place_role = ""
 ...
 #       return bibliographic_id_list, person_list, place_list, title, volume_number, part_title, printing_date, printing_information
        ...
        if printing_date_raw:
            ...
            if re.match(year_pattern_isolated, printing_date_raw): # This means ca. 80% of the cases, when there is just a four-digit year
            ...
            elif re.search(year_pattern_brackets, printing_date_raw): # If there is a date given in brackets, it should be the preferred form to use
                ...
                if printing_date_divided[0]:
                    ...
                    match printing_date_divided[0]:
                        ...
                        case "vor ": # I include the year for 'before' in the string since I am not sure if it always means 'before Jan 1st'
            ...
            else: # This means that there the original datestring from the book that will hopefully contain a legible date
                ...
                if re.search(year_pattern_Arabic_in_text, printing_date_raw): #This means that if there is a four-digit date starting with '1' in the text and nothing in square brackets
                    ...
                    start_year = int(year_string)
                    end_year = int(year_string)
                if re.search(year_pattern_Roman_in_text, printing_date_raw): # This means that there is a string in Roman numerals starting with "M"
                    year_string = roman_numerals(re.search(year_pattern_Roman_in_text, printing_date_raw).groups()[0])
                    bi.date_string = year_string
...
def VD16_parsing(url_bibliography):    
    ...
    # NB: currently, this data is only available as HTML. I built this code for the original version, and it does nto fully work for the current Beta version.
    # Since it is planned to make export possible as MARCXML I do not want to sepnd much time making improvements here. 
...
#    person_single_pattern = r'(<a h.*?>)(.*?)(</a>)(.*?)(DE-588)?(.*)?(: Datensatz)?(.*)?'
    ...
    # Although some early 16th-century books give a day, the date field in the VD16 only contain a year, two years separated by "/" (two different years given in the book) "-"
    if printing_date_long != "":
        ...
        if printing_date_divided != []:
            if len(printing_date_divided[1]) == 2: #If for the second year only the last two digits are given
                printing_date_divided[1] = "15" + printing_date_divided[1]
            if int(printing_date_divided[0]) < int(printing_date_divided[1]): #if the first figure is lower (this is not always the case)
...
def ISTC_parsing(URL_bibliography):
    # A small problem: if there are several imprints, I take the place and printer information from all, but the date only from the last. 
    bibliographic_id_list = []
    person_list = []
    place_list = []
    author = ""
    date_pattern = r'(About |about |Before |before |Not before |not before |Shortly before |shortly before |Between |between |After |after |Not after \
        |not after |Shortly after |shortly after )?(\d{1,2} )?([A-Za-z\.]{3,5} )?(\d{4})?(/\d{2,4}|-\d{2,4})?( and |and )?(\d{1,2} )?([\w\.]{3,5} )?(\d{4})?'
    month_names = {"Jan. " : "January ", "Feb. " : "February ", "Mar. " : "March ", "Apr. ": "April ", "May " : "May ", "June " : "June ", "July " : "July ", "Aug. " : "August ", \
                    "Sep. " : "September ", "Sept. " : "September ", "Oct. " : "October ", "Nov. " : "November ", "Dec. " : "December "}
    month_numbers = {"Jan. " : 1, "Feb. " : 2, "Mar. " : 3, "Apr. ": 4, "May " : 5, "June " : 6, "July " : 7, "Aug. " : 8, \
    ...
    if "imprint" in istc_record_short:
        for step1 in istc_record_short["imprint"]: # this is iterated in case there are several imprints, I reckon
            printer_name_long = ""
            publisher_name_long = ""
            ...
            if "imprint_date" in step1:
                ...
                if printing_date_divided[5]:
                    date_between_indicator = printing_date_divided[5] # I wonder if I even need it - probably not. 
                ...
                if date_prefix == "" and date_year_to == "" and date_between_year == "": #If there is only one date                   
                    print("Only one year")
                    if date_year != "": #This is not the case if there is a date such as "Between Jan. and Oct. 1488"
                        string_year = date_year + " "
                        start_year = int(date_year)
                        end_year = int(date_year)
                elif date_prefix != "" and date_year_to == "" and date_between_year == "": #If there is only one date, that is not exact
#                    print("only one year, but prefixes")
                    match date_prefix:
                        ...
                        case "Shortly before " | "shortly before ":
                            string_prefix = "shortly before " # I am not sure if I will suppress this eventually??
                            string_year = date_year + " "
                            start_year = int(date_year) - 1
                            end_year = int(date_year)
                        ...
                        case "Not after " | "not after ":
                            ...
                            start_year = int(date_year) - 2
                            end_year = int(date_year)
                elif (date_prefix == "Between " or date_prefix == "between ") and date_between_year != "":
#                    print("timespan with between")
                    string_prefix = "between "
                    string_year_between = date_between_year

                    if date_year != "": #if the start and and the end of the time-span are not in the same year
...
#                        print("start of timespan has no year")
                    ...
                    if date_year_to[0] == "-": # Question: is this really different from 'between'? (I would guess that '-' could mean that the production took from a to b, and
                        ...
                        if date_prefix == "About " or date_prefix == "about ":
                            string_prefix = "about " #in this case I don't change the dates, just add the 'about'
                        else:
                            string_prefix = ""

                    
                    if date_year_to[0] == "/": #This should mean exact dates from countries where the year started in March/April, 
                    #so that a date like January 1490 is Jan 1491 in our calendar
                        start_year = int(date_year)+1
                        end_year = int(date_year)+1
                        string_year = date_year + " (in modern calendar " + str(int(date_year)+1) + ")"
                ...
                elif date_between_year != "": # Thus, there is no end month but an end year in this case, the end month has to be December.
                    end_month = 12
                ...
                if date_month != "": 
                    ...
                    if end_month == 0: #thus, if there is no indication of an end month, as in a timespan
                ...
                elif date_between_year != "": # Thus, there is no end day but an end year in this case, the end month has to be December.
                    if end_month in [1, 3, 5, 7, 8, 10, 12]:
                        end_day = 31
                    if end_month in [4, 6, 9, 11]:
                        end_day = 30
                    if end_month == 2 and end_year%4 == 0: # In the Julian calendar, 1500 is a leap year - but will Python allow that? 
                        end_day = 29
...
#            bi.printing_date = bi.date_string + " (" + bi.date_start.isoformat()[0:10] + " - " + bi.date_end.isoformat()[0:10] + ")"
            ...
            if imprint_name_long:
                ...
                imprint_name_long = imprint_name_long.replace("and for", "and") # sometimes, the "for" is repeated for a second publisher, what is confusing
                print("imprint_name_long after replacement: " + imprint_name_long)
                if " for " in imprint_name_long: # in this case there are both a printer and a publisher
                    ...
                    bi.places.append(pl_duplicate) # I append the place again, this time as place of publication
                    # I could instead replace the string for the role with a list, but this is a lot more work, so I have to think 
                ...
                if printer_name_long:
                    ...
                    if " and " in printer_name_long: #in this case, there are two printers
                        ...
                        while printer_counter < len(printer_name_long_divided):
                            ...
                            if " " in printer_name: #if there is a blank inside the name - hence it is more than just a Christian name
                                pe.name = printer_name
                            else: # If there is only a Christian name

                                if printer_name_long_divided[printer_counter+1]:
                                    next_printer = printer_name_long_divided[printer_counter+1].strip()
                                    # if there is a next printer in the list whose name has at least two words
                                    if " " in next_printer:
                                ...
                                else: # if there is no next printer, or the next printer does not have a surname, either
                                        pe.name = printer_name                        
                            pe.role = "prt"                
                            bi.persons.append(pe)
                ...
                if publisher_name_long:
                    ...
                    if " and " in publisher_name_long: #in this case, there are two publishers
                        ...
                        while publisher_counter < len(publisher_name_long_divided):
                            ...
                            if " " in publisher_name: #if there is a blank inside the name - hence it is more than just a Christian name
                                pe.name = publisher_name
                            elif "himself" in publisher_name:
                                pe.name = printer_name_long # in this case there can only be one printer
                            else: # If there is only a Christian name
                                if publisher_name_long_divided[publisher_counter+1]:
                                    next_publisher = publisher_name_long_divided[publisher_counter+1].strip()
                                    # if there is a next publisher in the list whose name has at least two words
