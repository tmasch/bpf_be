...
from bpf.parsing import parse_canvas
...
def parse_manifests_bsb(manifest) -> classes.Graph:
    ...
    manifest_graph=classes.Graph()
    meta=classes.Node()
    meta.type="Metadata"

    if "license" in manifest:
        meta.set_attribute("license",manifest["license"][0])

    location = ""
    location=get_metadata_value_by_label(metadata,"Standort")
    location_pattern = r'(.*)(--)(.*)'
    if location:
        meta.set_attribute("location",location[0])
        location_divided = re.match(location_pattern, location[0])
        org=classes.Node()
        org.type="Organisation"
        org.name=location_divided.groups()[0]
        org.set_attribute("role","col")
        manifest_graph.nodes.append(org)
        link=classes.Edge()
        link.entityA=meta
        link.entityB=org
        manifest_graph.edges.append(link)

        meta.set_attribute("shelfmark",location_divided.groups()[2].lstrip())

    meta.external_id.extend(parse_bibliographic_ids_bsb(metadata))
#    print(meta)
    manifest_graph.nodes.append(meta)
    return manifest_graph


def parse_bibliographic_ids_bsb(metadata):
    bibliographic_ids = []
    for id in get_metadata_value_by_label(metadata,"Identifikator"):
#        print("text "+id.text)
        # Sometimes, a link to a bibliographic record is given, 
        # with the bibliographic ID being the 'friendly text' in 
        # the link; sometimes there is only a string with the 
        # bibliographic ID; and in other cases a string with 
        # a provisional and essentially useless bibliographic ID
        #  that needs to be ignored
        if "https" in id: #if there is a link
            bid =classes.ExternalReference()
            t=etree.fromstring(id)
            s=t.find("a").text
            a=s.split(" ")
            bid.name=a[0]
            bid.external_id=a[1]
            bid.uri=t.find("a").attrib["href"]
            bibliographic_ids.append(bid)
#            print(t)
#            print("a")
#            print(t.find("a").text)
#            print(t.find("a").attrib["href"])
#            bibliographic_id_pattern = r'(.*\')(.*)(\'>)([A-Za-z0-9]*)( )(.*)(</a.*)'
#            bibliographic_id_divided = re.match(bibliographic_id_pattern, id)
...
# does not work:
        # elif len(id)> 0 and "-" not in id: #if there is no link
        #     bibliographic_id_pattern_reduced = r'([A-Za-z0-9]*)( )(.*)'
        #     bibliographic_id_divided = re.match(bibliographic_id_pattern_reduced, id)                
...
def get_metadata_value_by_label(metadata,key):
    values=[]
    for m in metadata:
        labels = m["label"]
        for l in labels:
