...
(a) searching for persons, organisation, and places mentioned in book titles in the database and 
(b) creating new such records by parsing the GND
"""
...
ID-number, otherwise for the name as string, and if this fails, for the name as key-words).
    """
# I think that there are problems with the search strategy -
# I hope it was not wrong in my original version.
# 1: is there a GND ID known (never from ISTC; but from VD17/18, and perhaps from VD16)
#   if yes:
#       2: is the GND ID already in the database?
#           if yes:
#               3: is the Person_type1 (should be turned into something different)
#                   compatible with the role demanded here?
#                       if yes:
...
#                           Unless the user makes a manual search with a different result,
#                           the person_type1 corresponding to the role will be added.
#           if no:
#               Search for the record with this ID in the GND; parse it,
#                   and present it as only search result
#               Unless the user manually searches with a different result,
#                   the record will be added with a
#               person_type_one according to the role it has here.
#   if no:
#       4: Are records with this name already in the database?
#           (search both for name_preferred and name_variant)
#           if yes:
...
#                   if yes:
#                       make a search in ULAN (currently only OR search for individual words,
#                       this is not ideal)
#                       present list of results
#                       save selected result with person_type_1 'Artist'
#                   if no:
#                       7. Is the name as a string found in the GND?
#                           if yes:
#                               present list of results, save selected result
#                               with person_type1 corresponding to role
#                           if no:
...
#                                       > prompt user to give an ID from the GND or another database
#                                           or create a stub record with the information available
# Or should all the stuff with Role/Person_type done after the search
# (naturally, there can only be a discrepancy if a record is already in the database.)
...
#    candidates = []
    internal_id_person_type1_needed =  \
        parsing_helpers.map_role_to_person_type(role_in.get_attribute("role"))
...
#    gnd_id_in=role_in.entity_and_connections.entity.gnd_id
    ...
    if gnd_id_in:
        xx = classes.Role.find(classes.Role.entity_and_connections.entity.gnd_id \
                               == gnd_id_in,fetch_links=True)
...
#        role_in_db[0].comment="(in Database)"
    ...
    #             person.internal_id_person_type1_comment = \
    #                  "This person is currently catalogued as " + \
    #               person_type1_present + ", but not as '" + \
    #               person.internal_id_person_type1_needed + "'.
    #               The latter will be added if this record has been saved. "
...
#        else:
#            if person.id_name == "GND":
#           # I will have to create similar things for other authority files
        ...
        print(authority_url)
#        role_in.entity_and_connections.connected_persons = \
#               await find_and_parse_person_gnd(authority_url)
        person_found=True
        # This variable doesn't exist. Furthermore, there has not yet been any search
        print(role_in.model_dump())
...
#         candidates_result = coll.find({"name_preferred" : person.name}, \
#           {"id": 1, "name_preferred" : 1, "person_type1" : 1})
...
#             candidate.preview = candidate.name_preferred + " (in Database)"
#               # The years should be added once I have them
...
#         candidates_result = coll.find({"name_variant" : person.name}, \
#               {"id": 1, "name_preferred" : 1, "person_type1" : 1})
# #        candidates_result = db_actions.find_person(person,"name_variant")
#         #I search first for the preferred names
#           (assuming that it is more likely there will be a good match,
#           and only later for the variants)
#         for candidate_result in candidates_result:
#             candidate = classes.Person()
#             candidate.internal_id = candidate_result["id"]
#             candidate.name_preferred = candidate_result["name_preferred"]
#             candidate.internal_id_person_type1 = candidate_result["person_type1"]
#             candidate.preview = candidate.name_preferred + " (in Database)"
#                   # Also here, the years should be added
...
#                 if person.internal_id_person_type1_needed not in \
#                   candidate.internal_id_person_type1:
#                     person_type1_present = ""
#                     for t in candidate.internal_id_person_type1:
#                         person_type1_present = person_type1_present + "' and '" + t + "'"
#                     person_type1_present = person_type1_present[5:]
#                     candidate.internal_id_person_type1_comment = \
#                   "This person is currently catalogued as " + \
#                       person_type1_present + ", but not as '" + \
#                       person.internal_id_person_type1_needed + "'.
#                   The latter will be added if this record has been saved. "
    ...
    if not found_person and role_in.get_attribute("gnd_id"):
        ...
        authority_url = r'https://services.dnb.de/sru/authorities?'\
            + r'version=1.1&operation=searchRetrieve&query=Per%3D' + person_name_search + \
                r'%20and%20BBG%3DTp*&recordSchema=MARC21-xml&maximumRecords=100'
        print(authority_url)
#        person_in.person_candidates = parse_person_gnd(authority_url)

    if not found_person:
        # This search should only happen if the search before did not work.
        # It should likewise exclude artists.
            # if not person_in.person_candidates:
            # #if still nothing has been found,
            # a keyword search is performed instead of a string search.
        ...
        for word in name_divided:
            if word != "": #necessary, otherwise there will be error messages
                search_phrase = r"Per=" + word + r"%20and%20"
                # I don't get it, but the thing only works if the "="
                # is written as such and not as Percent code. Above, it is different.
                name_query = name_query + search_phrase
        authority_url = r'https://services.dnb.de/sru/authorities?'\
            + r'version=1.1&operation=searchRetrieve&query=' + name_query + \
                r'BBG%3DTp*&recordSchema=MARC21-xml&maximumRecords=100'
#        print(authority_url)
...
#     if len(person_in.person_candidates) == 1:
#       # If there is only one entry for this person, it is by default selected
#       (although the user can also run a new search, once this is established)
...
for persons connected to a book.
Normally, they are parsed with gnd_parsing_person - but beforehand it is checked
if they are already in Iconobase and have not been found for whatever reason.
Currently all records must come from the GND - if other authority files are included,
...
#    person_found = coll.find_one({"external_id": {"$elemMatch": \
#   {"name": "GND", "id": new_authority_id}}}, {"id": 1, "person_type1": 1, "name_preferred": 1})
    person = classes.Node()
    ...
    if person_found:
        ...
        potential_person.preview = person_found["name_preferred"]
        # The date should be added, but I first have to write how it is to be parsed
        ...
        if internal_id_person_type1_needed not in potential_person.internal_id_person_type1:
            ...
            for t in potential_person.internal_id_person_type1:
                person_type1_present = person_type1_present + "' and '" + t + "'"
            person_type1_present = person_type1_present[5:]
            potential_person.internal_id_person_type1_comment = \
                "This person is currently catalogued as " + person_type1_present + \
            ", but not as '" + internal_id_person_type1_needed + \
                "'. The latter will be added if this record has been saved. "
...
#    candidates = []
    ...
    organisation.internal_id_org_type1_needed = \
        parsing_helpers.map_role_to_organisation_type(organisation.role)
    organisation.chosen_candidate = 999 # For some reason, this must not be empty
    if organisation.id:
#        organisation_found = coll.find_one({"external_id": {"$elemMatch": \
#           {"name": organisation.id_name, "id": organisation.id}}}, \
#          {"id": 1, "name_preferred": 1, "org_type1": 1})
        ...
        if organisation_found:
            ...
            if org_type1_needed not in organisation.internal_id_org_type1:
                ...
                organisation.internal_id_org_type1_comment = \
                    "This organisation is currently catalogued as " + org_type1_present + \
                        ", but not as '" + org_type1_needed + \
                            "'. The latter will be added if this record has been saved. " 

        else:
            if organisation.id_name == "GND":
                # I will have to create similar things for other authority files
                authority_url = r'https://services.dnb.de/sru/authorities?'\
                    + r'version=1.1&operation=searchRetrieve&query=NID%3D' + \
                    organisation.id + \
...
#        candidates_result = (coll.find({"name_preferred" : organisation.name}, \
#       {"id": 1, "name_preferred" : 1, "org_type1" : 1}))
...
#        candidates_result = coll.find({"name_variant" : organisation.name}, \
#           {"id": 1, "name_preferred" : 1, "org_type1" : 1})
#           #I search first for the preferred names (assuming that it is more likely
#            there will be a good match, and only later for the variants)
        ...
        for candidate_result in candidates_result:
            ...
            candidate.internal_id_org_type1 = candidate_result["org_type1"]
            candidate.preview = candidate.name_preferred + " (in Database)"
            # Maybe I add other information to it later.
            print("orgtype1 in search for name_variant: ")
            print(candidate.internal_id_org_type1)
            candidate_duplicate = False # This is needed to avoid having a candidate listed twice,
            # it is rather longwided since I cannot use a simple if ... in thing in class instances.
...
#                print("Candidate not yet in list")
                organisation.potential_candidates.append(candidate)
            # The following is about a warning if the found organisations have the wrong type.
            # I could not try it out, since the VD17 always gives IDs of organisations,
            ...
            for candidate in organisation.potential_candidates:
                ...
                if organisation.internal_id_org_type1_needed not in candidate.internal_id_org_type1:
                    ...
                    for t in candidate.internal_id_org_type1:
                        org_type1_present = org_type1_present + "' and '" + t + "'"
                    org_type1_present = org_type1_present[5:]
                    candidate.internal_id_org_type1_comment = \
                        "This organisation is currently catalogued as "\
                          + org_type1_present + ", but not as '" + \
                            organisation.internal_id_org_type1_needed + \
                                "'. The latter will be added if this record has been saved. "
        ...
        if not organisation.potential_candidates: #if nothing has been found in the database
            ...
            authority_url = r'https://services.dnb.de/sru/authorities?'\
                + r'version=1.1&operation=searchRetrieve&query=Koe%3D' + \
                organisation_name_search + \
                    r'%20and%20BBG%3DTb*&recordSchema=MARC21-xml&maximumRecords=100'
            organisation.potential_candidates = find_and_parse_organisation_gnd(authority_url)
            ...
            for word in name_divided:
                if word != "":
                    search_phrase = r"Koe=" + word + r"%20and%20"
                    # I don't get it, but the thing only works if the "=" is written as such
                    # and not as Percent code. Above, it is different.
                    name_query = name_query + search_phrase
#                    print ("name query:" + name_query)
            authority_url = r'https://services.dnb.de/sru/authorities?'\
                + r'version=1.1&operation=searchRetrieve&query=' + \
                name_query + r'BBG%3DTb*&recordSchema=MARC21-xml&maximumRecords=100'
            new_potential_candidates = find_and_parse_organisation_gnd(authority_url)
            for candidate in new_potential_candidates:
                if candidate not in organisation.potential_candidates:
                    #I need this distinction because I perform both a string and a keyword search,
                    # and both may yield the same results
                    organisation.potential_candidates.append(candidate)
    if len(organisation.potential_candidates) == 1:
        # If there is only one entry for this organisation, it is by default selected
        # (although the user can also run a new search, once this is established)
...
def identify_additional_organisation(new_authority_id, role):
    """
This function is used for any additional authority records that are suggested
as identifications for organisations connected to a book.
Normally, they are parsed with gnd_parsing_organisation - but beforehand it is checked
if they are already in Iconobase and have not been found for whatever reason. 
Currently all records must come from the GND - if other authority files are included,
this function has to be changed. 
    """
...
#    org_found = coll.find_one({"external_id": {"$elemMatch": \
#   {"name": "GND", "id": new_authority_id}}}, {"id": 1, "name_preferred": 1, "org_type1" : 1})
    ...
    if org_found:
        ...
        potential_org.preview = org_found["name_preferred"]
        # The date should be added, but I first have to write how it is to be parsed
        ...
        if internal_id_org_type1_needed not in potential_org.internal_id_org_type1:
            ...
            for t in potential_org.internal_id_org_type1:
                org_type1_present = org_type1_present + "' and '" + t + "'"
            org_type1_present = org_type1_present[5:]
            potential_org.internal_id_org_type1_comment = \
                "This organisation is currently catalogued as " + org_type1_present + \
                    ", but not as '" + internal_id_org_type1_needed + \
                        "'. The latter will be added if this record has been saved. "
        else:
        ...
        authority_url = r'https://services.dnb.de/sru/authorities?'\
            + r'version=1.1&operation=searchRetrieve&query=NID%3D' + \
            new_authority_id + r'%20and%20BBG%3DTb*&recordSchema=MARC21-xml&maximumRecords=100'
...
async def identify_place(place):
    """
This function is used for every place named in the bibliographic record
(place of publishing / manufacture)
It will first search if a record for this place is already in the MongoDB database,
and then search in the GND
If there is an ID-number (internal or GND, the search is done for the ID-number,
otherwise for the name as string, and if this fails, for the name as key-words)
Note that the GND parser combined with it suppresses all records to regions -
if this mechanism is later also used for identifying regions, this might need to be changed
Since there are often many locations connected toa town (e.g., all villages in its district),
I increase the number of hits from the GND to 400 and sort them alphabetically. 
    """
    if place.role:
        print(place.role)
    else:
    ...
    if place.id:
#        place_found = coll.find_one({"external_id": {"$elemMatch": \
#   {"name": place.id_name, "id": place.id}}}, {"id": 1, "name_preferred": 1, "place_type1" : 1})
...
#            print(place.internal_id_place_type1)
            ...
            # included for all searches for names in Iconobase, but I don't build this
            # yet since there aren't any records in it that allow my to try it out.
            ...
            if place_type1_needed not in place.internal_id_place_type1:
                ...
                place.internal_id_place_type1_comment = "This place is currently catalogued as " +\
                    place_type1_present + ", but not as '" + place_type1_needed + "'. \
                    An additional record for " + place_type1_needed + \
                        " will be produced if this record is saved. "
        else:
            if place.id_name == "GND":
                # I will have to create similar things for other authority files
                authority_url = r'https://services.dnb.de/sru/authorities?'\
                    + r'version=1.1&operation=searchRetrieve&query=NID%3D' + place.id + \
...
#        candidates_result = coll.find({"name_preferred" : place.name}, \
#           {"id": 1, "name_preferred" : 1, "place_type1" : 1})
        candidates_result = db_actions.find_place(place,"name_preferred")
        for candidate_result in candidates_result:
            candidate = classes.Node()
            candidate.internal_id = candidate_result["id"]
            candidate.name_preferred = candidate_result["name_preferred"]
            # I need this to create previews for places of making
            print("candidate found through name search in database (preferred name)" \
...
#        candidates_result = coll.find({"name_variant" : place.name}, \
#       {"id": 1, "name_preferred" : 1, "place_type1" : 1})
#       #I search first for the preferred names (assuming that it is more likely
#       there will be a good match, and only later for the variants)
        ...
        for candidate_result in candidates_result:
            candidate = classes.Node()
            candidate.internal_id = candidate_result["id"]
            print("candidate found through name search in database (variant name name)" + \
                  candidate.internal_id)
            candidate.preview = candidate_result["name_preferred"] + " (in Database)"
            candidate.internal_id_place_type1 = candidate_result["place_type1"]
            ...
            for candidate in place.potential_candidates:
                ...
                if place.internal_id_place_type1_needed not in candidate.internal_id_place_type1:
                    ...
                    candidate.internal_id_place_type1_comment = \
                        "This place is currently catalogued as " + \
                        place_type1_present + ", but not as '" + \
                            place.internal_id_place_type1_needed + "'. \
                    An additional record for " + place.internal_id_place_type1_needed + \
                        " will be produced if this place is selected and the record is saved. "
        ...
        if not place.potential_candidates: #if nothing has been found
            ...
            authority_url = r'https://services.dnb.de/sru/authorities?'\
                + r'version=1.1&operation=searchRetrieve&query=Geo%3D' + place_name_search + \
                    r'%20and%20BBG%3DTg*&recordSchema=MARC21-xml&maximumRecords=100'
...
#       I actually do not believe that one needs a words search for places
            ...
            for word in name_divided:
                if word != "":
                    search_phrase = r"Geo%3D" + word + r"%20and%20"
                    # I don't get it, but the thing only works if the "="
                    # is written as such and not as Percent code. Above, it is different.
                    name_query = name_query + search_phrase
                    print ("name query:" + name_query)
            authority_url = r'https://services.dnb.de/sru/authorities?'\
                + r'version=1.1&operation=searchRetrieve&query=' + name_query + \
                    r'BBG%3DTg*&recordSchema=MARC21-xml&maximumRecords=100'
    ...
    place.potential_candidates = \
        sorted(place.potential_candidates, key = lambda candidate : candidate.preview)
    if len(place.potential_candidates) == 1:
        # If there is only one entry for this person, it is by default selected
        # (although the user can also run a new search, once this is established)
...
This function is used for any additional authority records that are suggested
as identifications for organisations connected to a book.
Normally, they are parsed with gnd_parsing_place - but beforehand it is checked
if they are already in Iconobase and have not been found for whatever reason. 
Currently all records must come from the GND - if other authority files are included,
this function has to be changed. 
    """
...
#    place_found = coll.find_one({"external_id": {"$elemMatch": \
#   {"name": "GND", "id": new_authority_id}}}, {"id": 1, "name_preferred": 1, "place_type1" : 1})
    place=classes.Node
...
#        print(place_found)
        ...
        if internal_id_place_type1_needed not in potential_place.internal_id_place_type1:
            ...
            for t in potential_place.internal_id_place_type1:
                place_type1_present = place_type1_present + "' and '" + t + "'"
            place_type1_present = place_type1_present[5:]
            potential_place.internal_id_place_type1_comment = \
                "This place is currently catalogued as " \
                + place_type1_present + ", but not as '" \
                + internal_id_place_type1_needed + "'. An additional record for " \
                    + internal_id_place_type1_needed + \
                        " will be produced if this place is selected and the record is saved. "
        potential_places_list.append(potential_place)
    else:
        authority_url = r'https://services.dnb.de/sru/authorities?'\
            + r'version=1.1&operation=searchRetrieve&query=NID%3D' + new_authority_id + \
                r'%20and%20BBG%3DTg*&recordSchema=MARC21-xml&maximumRecords=100'
...
async def find_and_parse_person_gndid(gnd_id):
    """
    I am not sure what the purpose of this function is. 
    """
    authority_url = r'https://services.dnb.de/sru/authorities?'\
        + r'version=1.1&operation=searchRetrieve&query=NID%3D'\
                    + gnd_id\
...
async def find_related_persons(gnd_id):
    """
    I have to admit that I am not sure what the purpose of
    this function is. 
...
def gnd_record_get_gnd_internal_id(record):
    ...
    since this ID always seems to appear in one of the subfields of 035, too. 
    """
    external_references=[]
...
def gnd_record_get_gnd_id(record):
    ...
    since all entries in 035 are now read in gnd_record_get_external_references
    """
    gnd_id=""
...
def gnd_record_get_external_references(record):
    """
    parses the external references (035)
    keeps all references in subfield a 
    (thus the internal ID and the public ID)
    and those old IDs in subfield z that are
    no repetitions. 
    """
    external_references=[]
    id_list = [] # for deduplication
    datafields = find_datafields(record,"035")
    for datafield in datafields:
        ...
        if subfields:
            ...
            if subfields[0][0:8] == "(DE-588)":
                ...
                id_list.append(external_reference.external_id)
                print("id_list")
                print(id_list)
                external_reference.name = "GND"
        ...
        if subfields:
            if subfields[0][0:9] in ["(DE-588a)", "(DE-588b)", "(DE-588c)"] \
                and subfields[0][9:] not in id_list:
                external_reference = classes.ExternalReference()
                external_reference.external_id=subfields[0][9:]
                external_reference.name = "GND old"
            if subfields[0][0:8] == "(DE-588)" and subfields[0][8:] not in id_list:
                external_reference = classes.ExternalReference()
                external_reference.external_id=subfields[0][8:]
                external_reference.name = "GND old"
        for extant_external_reference in external_references:
            ...
            external_references.append(external_reference)
    print("external references collected at the end:")
    print(external_references)
    return external_references
...
def gnd_record_get_sex(record):
    """
    Parses the sex of a person (field 375)
    """
    sex = ""
...
def gnd_record_get_person_name_variant(record, comments):
    ...
    for datafield in datafields:
        ...
        subfields = find_subfields(datafield,"c")
        if subfields:
            if " von " in subfields[0] or " of " in subfields[0] \
                or " de " in subfields[0] or "," in subfields[0]:
...
def gnd_record_get_connected_persons(record):
    """
    Parses references to persons connected to a record (field 500)
    """
    ...
    # How should this be connected to the person record?
    # As long as there is no connection, it is not called,
    ...
    for datafield in datafields:
        ...
        if subfields:
            if "http" not in subfields[0]:
                ...
                # Once one can give connection_types for both directions,
...
def gnd_record_get_connected_orgs(record):
    """
    Parses references to organsations in a GND record (field 510)
    """
    connections = []
...
def gnd_record_get_connected_places(record):
    """
    Parses references to places in a GND record (field 551)
    """
    connections = []
    ...
    for datafield in datafields:
        ...
        if subfields:
            if "http" not in subfields[0]:
                ec.connection_type = subfields[0]
                # Once one can give connection_types for both directions,
                # here the correct connection types in both directions
                # have to be created with the help of all the
...
def gnd_parsing_place_part_of_list(root):
    """
Unfortunately, the search for places often yields several hundred results.
Since the normal search function only downloads 100 results
per bathc, I may need to run it several times,
and hence I have to call the search several times. 
In order to do so, I had to divide the function gnd_parsing_place.
The function with this name now only launches one or more search options to bring back all results
and later filters out many irrelevant results that I annoyingly cannot exclude from the search. 
The longest part of the function the actual parsing of the XMl results,
is moved to this function gnd_parsing_place_part_of_list. 
    """
    ...
    for record in records:
        ...
        if(("gik" in entity_types or "giz" in entity_types or "gxz" in entity_types) \
           and "gil" not in entity_types):
            ...
            # If I want to use this function later also for searching e.g., countries,
    ...
    for datafield in datafields:
        ...
        if subfields:
            ...
            if external_reference.name == "geonames":
                external_reference.uri = "https://sws.geonames.org/" + \
...
#authority_url = r'https://services.dnb.de/sru/authorities?'\
# + r'version=1.1&operation=searchRetrieve&query=NID%3D' + organisation.id + \
# r'%20and%20BBG%3DTb*&recordSchema=MARC21-xml&maximumRecords=100'
#print(authority_url)
...
#authority_url = r'https://services.dnb.de/sru/authorities?'\
# + r'version=1.1&operation=searchRetrieve&query=Koe%3DBamberg,%20Staatsbibliothek%20%20and'\
# + r'%20BBG%3DTb*&recordSchema=MARC21-xml&maximumRecords=100'
#record = gnd_parsing_organisation(authority_url)
#record = organisation_identification(organisation)
#print(record)
#authority_url = r'https://services.dnb.de/sru/authorities?'\
# + r'version=1.1&operation=searchRetrieve&query=NID%3D4086808-4%20and'\
# + r'%20BBG%3DTg*&recordSchema=MARC21-xml&maximumRecords=100'
#place = Place()
#place.id_name = "GND"
#place.id = "4086808-4"
...
async def identify_making_process(making_processes):
    """
    This module has bene primarily made for parsing information about the making process
    of a manuscript or printed book that had been entered manually during the ingest process.
    If such information is added when working on individual records, it might be possible
    to adopt this module, but one would probably rather go directly
    to parsing the individual parts of it. 
    Manuscripts have by default one Making Process, Printed Books two
    (in theory three, but the third, printing, is already defined by bibliographic data
    and hence would not be added here). 
    Each making process has a number (currently as integer,
    although I wonder if one should not rather use a string to allow for e.g. 1a, 1b etc), 
    a process_type (the activity, e.g. design, blockcutting, etc.),
    and a process qualifier (e.g., none, attributed, follower of, etc.) - these two 
    will eventually probably belong drop-down fields. 
    It also has three fields that need parsing - one for a person (normally the artist),
    one for a place, and one for a date. 
    The person will be parsed similar to persons conntected to a book -
    but instead of the GND, the Getty ULAN would be the preferred source of information. 
    The place will be parsed as places conntected to a book (also here historical names),
    and the GND will also here be the principal source. 
    The date will be entered according to relatively simple rules
    that the editors would have to learn and would be parsed in a separate routine. 
...
def find_datafields(record,tag_id):
    """
    Function for getting one field from a MarcXML record
    """
    datafields=record.findall("{*}recordData/{*}record/{*}datafield[@tag='"+tag_id+"']")
    return datafields
...
def find_subfields(datafield,subfield_id):
    """
    Function for getting one subfield from the field of a MarcXML record. 
    """
    r=[]
    subfields=datafield.findall("{*}subfield")
    for subfield in subfields:
